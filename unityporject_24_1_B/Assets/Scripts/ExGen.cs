...
public class ExGen : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            GameObject Temp = Instantiate(item);            //오브젝트를 생성하는 함수 <Instantiate> ,GameObject를 임로 저장할 Temp 정의 선언
            Temp.transform.position += new Vector3(0.0f, Random.Range(0, 4), 0.0f);  //랜덤으로 0.3의 값으로 위치를 바꾼다.
