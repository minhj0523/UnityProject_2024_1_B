...
public class ExRayCast : MonoBehaviour
{
    ...
    void Update()
    {
        CheckTime += Time.deltaTime;                   //프레임 누적되어시간을 계산하게 한다
        GameTime -= Time.deltaTime;                    //프레임 시간을 제거하여 30초 -> 0초로 가게 한다.

        if(GameTime <= 0)                             //0초가 되면 
        {
            PlayerPrefs.SetInt("Point", Point);        //유니티에서 제공하는 저장함수
        ...
        {
            int RamdomX = Random.Range(0, 12);         //0~11사이의 랜덤값을 뽑아넨다.
            int RandomY = Random.Range(0, 12);         //0~11사이의 랜덤값을 뽑아넨다.
            GameObject temp = Instantiate(TargetObject);            //Instantiacte 함수를 통해서 프리팹을 생성한다.
            temp.transform.position = new Vector3(-6 + RamdomX, -6 + RandomY, 0);      //랜덤값을 더해서 -6 ~ 5 사이의 값을 랜덤하게 배치
        ...
        if (Input.GetMouseButtonDown(1))       //마우스 오른쪽 버튼이 눌렸을 경우
        {
            ...
            {
                Debug.Log(hit.collider.gameObject.name);         //검출된 오브젝트 이름을 출력해 준다.
