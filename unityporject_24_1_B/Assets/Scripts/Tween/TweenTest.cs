...
public class ExTween : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Sequence sequence = DOTween.Sequence();               //시퀀스를 생성한다. (앞의 Tween이 끝나면 다음 트
        ...
        //transform.DOMoveX(0, 0.5f).SetEase(Ease.OutBounce).OnComplete(DeactivateObject);           //동할때 효과를 추가 시킬 수 있다.   (Ease.OutBounce)
        //transform.DOShakeRotation(0.5f, new Vector3(0, 0, 5), 10, 90);      //이동하는 동안 회전 충격 추가


        sequence = DOTween.Sequence();               //시퀀스를 생성한다.
